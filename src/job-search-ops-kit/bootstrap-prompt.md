# Go-To-Job-Market: Bootstrap Prompt
...
is the generalized version, with the mistakes and fixes baked in so you
...
walk them through connecting at least Gmail early, because most of what makes
...
syntax via your Notion tools, and see `notion-gotchas.md` in this kit for
...
avoid, which is what the tailoring skill weighs Priority against) and a
...
than a flat list of links, so it stays navigable as it grows past a
...
scratch. It's the single highest-leverage page in the whole system once it's
...
`notion-gotchas.md` are real ones that cost real time to figure out, and are
